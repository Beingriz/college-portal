...
.border .form form table tr td label{padding: 15px; font-family: sans-serif; font-size: 25px; }
.border .form a{color: red; padding: 15px; font-size: 25px;}
.border .form a:hover{color: green;}
...
<body >
  ...
  $host = "localhost";
   ...
   $N_ID = $_GET['id'];
   $open = "Select * from Student_notice_box where Notice_ID='$N_ID'";
...
<form method="POST">
    <table width="78%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <a href="Student-Notice.php">Close</a>
                    </div>
                  </td>
              </table>
              <?php 
...
{
  $idno = $_POST['notice_id'];
  $read = "Read";
  $readnotice = "UPDATE student_notice_box SET Status='$read' Where Notice_ID='".$idno."'  ";
  $sql_read = mysqli_query($conn, $readnotice);

  if($sql_read)
  {
              echo '<script type="text/javascript">window.location=\'Student-Notice.php\';</script>';
  }
  else
  {
      echo '<script type="text/javascript">alert("Unable to Read");</script>';
